...
namespace DBDataGenLibrary
{
    public class TeamGenerator
    {
        public static void Generate(NpgsqlConnection conn, ref List<long> teamIds, long divisionId, List<long> leaderIds, List<long> juniorLeaderIds, char gender, DateTime startDate)
        {
            ...
            string history = "INSERT INTO leader_team_history (team_id, leader_id, is_junior, join_date) VALUES ";
            ...
            foreach (long leader_id in leaderIds)
            {
                ...
                history += string.Format("({0}, {1}, {2}, '{3}'),", team_id, leader_id, false, startDate);
                history += string.Format("({0}, {1}, {2}, '{3}')", team_id, juniorLeaderIds[leaderIds.IndexOf(leader_id)], true, startDate);
            }
